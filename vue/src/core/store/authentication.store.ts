...
import { credentials, JWTDto } from '@/core/apis/dto/JWT.dto'
...
const useAuthStore: StoreDef = defineStore({
  ...
  actions: {
    ...
    async updateProfile(props: UpdateProfileDto): Promise<void> {
      const dto: JWTDto = await API.settings.profile.updateProfile(props)
      this.authenticate(dto)
    },

    async updateAvatar(props: File): Promise<void> {
      const dto: JWTDto = await API.settings.profile.updateAvatar(props)
      this.authenticate(dto)
    },

    async removeAvatar(): Promise<void> {
      const dto: JWTDto = await API.settings.profile.removeAvatar()
      this.authenticate(dto)
    }
  }
})
