...
type FormValues = { current_password: string; password: string; confirm_password: string }
type PropType = {
  authenticatedUser: UserDto | undefined
  callback: (props: FormValues) => Promise<void>
}
...
const toggle = (_event: MouseEvent): void => {
  state.open = !state.open
}

const onSubmit = validateUtil.getSubmitFn(validationSchema, async (values: FormValues) => {
  state.loading = true
