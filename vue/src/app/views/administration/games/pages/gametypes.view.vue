...
const sort: Array<SortOptions> = [
  { sort: 'gametype.createdAt', order: Order.ASC, label: 'forms.oldest' },
  { sort: 'gametype.createdAt', order: Order.DESC, label: 'forms.newest' },
  { sort: 'gametype.name', order: Order.ASC, label: 'Name' }
]

const { getPaginated, create, remove, update } = GameLibraryService.gametypes
...
<template>
  <TablePaginated
    ...
    :caption="t('administration.games.gametypes.caption', { showing: response.data.length }, response.meta.itemCount)"
  >
    <template v-slot>
      <button
