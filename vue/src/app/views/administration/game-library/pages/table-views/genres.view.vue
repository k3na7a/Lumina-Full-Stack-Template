...
const sort: Array<SortOptions> = [
  { sort: 'genre.createdAt', order: Order.ASC, label: 'forms.oldest' },
  { sort: 'genre.createdAt', order: Order.DESC, label: 'forms.newest' },
  { sort: 'genre.name', order: Order.ASC, label: 'forms.name' }
]
...
const { getPaginated, create, remove, update } = GameLibraryService.genres
  ...
  <ContentLayout title="administration.users.title" subtitle="administration.users.subtitle">
    <template #table>
      <TablePaginated
        ...
        :caption="
          t('administration.game-library.genres.caption', { showing: response.data.length }, response.meta.itemCount)
        "
      >
        <template v-slot>
          <button
