...
class games {
  ...
  public readonly updateCover = async (id: string, params: File): Promise<GameDto> => {
    const formData = new FormData()

    formData.append('cover', params)

    return this.$api
      .patch<GameDto>(
        `admin/game-library/games/${id}/cover`,
        formData,
        AxiosService.requestConfig({ token: this.$token.getItem(), content: 'multipart/form-data' })
      )
      .then((response: AxiosResponse) => new GameDto(response.data))
  ...
  public readonly findOne = async (slug: string): Promise<GameDto> => {
    return this.$api
      .get<GameDto>(`admin/game-library/games/${slug}`, AxiosService.requestConfig({ token: this.$token.getItem() }))
      .then((response: AxiosResponse) => new GameDto(response.data))
  }
